...
#define BOLD    "\033[1m"
#define BROWN   "\033[38;2;233;149;12m"
#define PINK    "\033[38;2;223;48;121m"
#define DRED    "\033[38;2;242;44;61m"
    ...
    public:
        ...
        void start()
        {
            adaptor_.start([this](const boost::system::error_code& ec) {
                ...
                {
                    CROW_LOG_ERROR << BOLD << RED << "Could not start adaptor: " << ec.message() << RESET;
                    check_destroy();
                }
            });
        }
        ...
        void handle()
        {
            ...
            if (req.check_version(1, 1)) // HTTP/1.1
            {
                ...
                CROW_LOG_INFO << GREEN << method_name(req.method) <<  " " << BROWN << req.url  << " " << DRED << boost::lexical_cast<std::string>(adaptor_.remote_endpoint()) << RESET << " " << (char)(req.http_ver_major + '0') << "." << (char)(req.http_ver_minor + '0') <<" " << CYAN << this << RESET;
            #else
                CROW_LOG_INFO << method_name(req.method) <<  " " << req.url  << " " << boost::lexical_cast<std::string>(adaptor_.remote_endpoint()) << " " << (char)(req.http_ver_major + '0') << "." << (char)(req.http_ver_minor + '0') <<" " << this;
            #endif
            ...
            if (!is_invalid_request)
            {
                ...
                if (!res.completed_)
                {
                    res.complete_request_handler_ = [this] {
                        this->complete_request(std::chrono::high_resolution_clock::now());
                    };
        ...
        void complete_request(std::chrono::high_resolution_clock::time_point startTime)
        {
            std::chrono::high_resolution_clock::time_point endTime = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(endTime - startTime);

            #ifdef CROW_FANCY_LOG
                CROW_LOG_INFO << (res.code >= 500 ? DRED : res.code >= 400 ? RED : res.code >= 300 ? YELLOW : res.code >= 200 ? GREEN : CYAN ) << res.code << " " << RESET << BROWN << req_.raw_url << " " << DRED << boost::lexical_cast<std::string>(adaptor_.remote_endpoint()) << RESET << " " << close_connection_ << " " << MAGENTA << duration.count() <<" ns "  << CYAN << this << RESET;
            #else
                CROW_LOG_INFO << res.code << " " << req_.raw_url << " " << boost::lexical_cast<std::string>(adaptor_.remote_endpoint()) << " " << close_connection_ << " " << duration.count() <<" ns ";
            #endif
