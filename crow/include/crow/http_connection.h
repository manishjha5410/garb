...
namespace crow
{
    ...
    public:
        ...
        void handle()
        {
            ...
            #else
                CROW_LOG_INFO << method_name(req_.method) <<  " " << req_.url  << " " << utility::lexical_cast<std::string>(adaptor_.remote_endpoint())  << " " << (char)(req_.http_ver_major + '0') << "." << (char)(req_.http_ver_minor + '0') <<" " << this;
            #endif
            ...
            if (!is_invalid_request)
            {
                ...
                if (!res.completed_)
                {
                    ...
                    res.complete_request_handler_ = [self] {
                        self->complete_request(std::chrono::high_resolution_clock::now());
                    };
        ...
        void complete_request(std::chrono::high_resolution_clock::time_point startTime)
        {
            ...
            #ifdef CROW_FANCY_LOG
                CROW_LOG_INFO << (res.code >= 500 ? DRED : res.code >= 400 ? RED : res.code >= 300 ? YELLOW : res.code >= 200 ? GREEN : CYAN ) << res.code << " " << RESET << BROWN << req_.raw_url << " " << DRED << utility::lexical_cast<std::string>(adaptor_.remote_endpoint()) << RESET << " " << close_connection_ << " " << MAGENTA << duration.count() <<" ns "  << CYAN << this << RESET;
            #else
